import { api } from "../api";
import {
  updateBackOfficeReportEndPoint,
  addReportEndPoint,
  getReportedStatisticsEndPoint
...
export const getReportedContent = (payload, type = "get", header = {}) => {
  ...
  switch (type) {
    case "get":
      ...
      break;
    case "search":
      apiURL = `/reports?reportContent=${payload.reportContent}&searchType=${
        payload.searchType
      }`;
      break;
    default:
...
export const getReportedStatistics = (payload, header = {}) =>
  api(baseUrl, header).get(
    getReportedStatisticsEndPoint + payload.reportContent
  );
