...
class AdminLogin extends Component {
  ...
  handleSubmit = event => {
    ...
    this.props.submitAdminLogin({ otp }).then(() => {
      ...
      console.log(
        "enumerations.adminRank.rank2 ",
        enumerations.adminRank.rank2
      );
      if (
        this.props.loginData &&
        this.props.loginData.user &&
        this.props.loginData.user.success === true
      )
        if (
          this.props.loginData.user.data.role === enumerations.adminRank.rank2
        ) {
          root_route = routes.BACK_OFFICE_CAMPAIGNS_ROUTE;
        } else if (
          this.props.loginData.user.data.role === enumerations.adminRank.rank3
        ) {
      ...
      if (
        ...
        this.props.loginData.error.data &&
        this.props.loginData.error.data.success === false
      )
